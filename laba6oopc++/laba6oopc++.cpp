...
void zad1() {
    ...
    ifstream bin_in("cppstudio.txt", ifstream::binary);
    if (!bin_in) {
        cerr << "Error: Невозможно открыть файл.\n";
    }
...
void zad2() {
    ifstream fin("cppstudio.txt");
    ofstream russianFile("russian.txt");
    ofstream englishFile("english.txt");
...
void zad5() {
    ...
    saveTableToFile("table.txt", table);

    // Считывание таблицы из файла
    vector<TeamStats> tableFromFile = readTableFromFile("table.txt");
...
void asd() {
    cout << "Заполняем изначальную талицу" << endl;
    ...
    do {
        ...
        case 1:
            ...
            break;
        case 2:
            tableFromFile = readTableFromFile("table.txt");
