...
def decode_image(encoded_img, output_txt, symbols_to_read, degree):
    ...
    while read < symbols_to_read:
        ...
        # text.write(base64.b64encode(chr(symbol)))
        # text.write(chr(symbol).encode())
        text.write(symbol.to_bytes(8, byteorder='little'))
...
inp = open("photo\\gta sa.bmp", 'rb'); output = open("secret.txt", 'wb')
output.write(inp.read())
inp.close(); output.close()

# encode_image('photo\\black.bmp', 'result.bmp', 'secret.txt', 4)

# decode_image('result.bmp', 'after_secret.txt', os.path.getsize('photo\\gta sa.bmp'), 4)

# with open('after_secret.txt', 'rb') as inp, open('3.1.stega.bmp', 'wb') as out:
#     out.write(inp.read())
